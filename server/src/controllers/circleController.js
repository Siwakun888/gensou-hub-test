...
import { CIRCLE_DIR, resolveUpload, removeUpload } from '../config/storage.js';
...
export async function createCircle(req, res) {
  ...
  try {
    ...
    const circle = await Circle.create({
      ...
      logo: req.file ? fileMeta(req.file) : null,
    });
    ...
    if (req.file) await removeUpload(CIRCLE_DIR, req.file.filename);
...
export async function updateCircle(req, res) {
  ...
  const previousLogo = circle.logo?.fileName;
  const clearingLogo = !req.file && req.body.removeLogo === 'true';

  if (req.file) circle.logo = fileMeta(req.file);
  ...
  if (previousLogo && (req.file || clearingLogo)) {
    await removeUpload(CIRCLE_DIR, previousLogo);
  }
...
export async function deleteCircle(req, res) {
  ...
  await removeUpload(CIRCLE_DIR, circle.logo?.fileName);
...
export async function getCircleLogo(req, res) {
  ...
  res.sendFile(resolveUpload(CIRCLE_DIR, circle.logo.fileName));
}
