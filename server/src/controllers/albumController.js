...
import { COVER_DIR, TRACK_DIR, resolveUpload, removeUpload } from '../config/storage.js';
...
export async function createAlbum(req, res) {
  ...
  try {
    const album = await Album.create({
      ...
      genre,
      description,
      cover: req.file ? fileMeta(req.file) : null,
    });
    res.status(201).json(serializeAlbum(album));
  } catch (err) {
    // Do not leave an orphaned cover on disk when validation rejects the album.
    if (req.file) await removeUpload(COVER_DIR, req.file.filename);
    throw err;
  }
}
...
export async function updateAlbum(req, res) {
  ...
  const previousCover = album.cover?.fileName;
  ...
  if (req.file) album.cover = fileMeta(req.file);
  ...
  if (previousCover && (req.file || clearingCover)) {
    await removeUpload(COVER_DIR, previousCover);
  }
  res.json(serializeAlbum(album));
}
...
export async function deleteAlbum(req, res) {
  ...
  await removeUpload(COVER_DIR, album.cover?.fileName);
  await Promise.all(album.tracks.map((t) => removeUpload(TRACK_DIR, t.audio.fileName)));
...
export async function getCover(req, res) {
  ...
  res.sendFile(resolveUpload(COVER_DIR, album.cover.fileName));
}
...
export async function addTrack(req, res) {
  ...
  album.tracks.push({
    ...
    audio: fileMeta(req.file),
  });

  try {
    await album.save();
  } catch (err) {
    await removeUpload(TRACK_DIR, req.file.filename);
    throw err;
  }

  const created = album.tracks.find((t) => t.audio.fileName === req.file.filename);
...
export async function deleteTrack(req, res) {
  ...
  track.deleteOne();
  await album.save();
  await removeUpload(TRACK_DIR, fileName);

  res.status(204).end();
}

// GET /api/albums/:id/tracks/:trackId/stream
// sendFile honours Range requests, so the browser can seek within a track.
export async function streamTrack(req, res) {
  ...
  res.sendFile(resolveUpload(TRACK_DIR, track.audio.fileName));
}
...
export async function downloadTrack(req, res) {
  ...
  res.download(resolveUpload(TRACK_DIR, track.audio.fileName), trackDownloadName(album, track));
}
