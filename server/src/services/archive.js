...
import { COVER_DIR, TRACK_DIR, resolveUpload } from '../config/storage.js';
 ...
 *
 * `files` is a list of { path, name }: where the file is on disk, and what it
 * should be called inside the archive.
 */
...
export function streamAlbumArchive(album, res) {
  ...
  const files = ordered.map((track) => ({
    path: resolveUpload(TRACK_DIR, track.audio.fileName),
  ...
  if (album.cover && files.length > 0) {
    files.push({
      path: resolveUpload(COVER_DIR, album.cover.fileName),
